...
main() {
  ...
  log "切換到新版本…"
  # ⚠️ `--force-recreate` 不能省，而且理由不直覺。
  #
  # `up -d` 是否重建容器，看的是「服務要用的映像有沒有變」，不是「執行中的容器
  # 跑的是不是那個映像」。這兩件事會分岔：2026-08-05 在正式機上實測，api 容器
  # 掛著一個**早已不存在**的映像 ID（`docker compose images api` 直接回
  # No such image），而 `sunny-api:latest` 沒變，於是 `up -d` 一次又一次回報
  # "Running" 而不動它。同一輪裡 caddy 因為前端 dist 變了、映像 ID 跟著變，
  # 就正常重建了——同一句指令，兩個服務兩種結果。
  #
  # 症狀還是那一個：部署回報成功、網站正常、跑的是舊的、沒有任何錯誤訊息。
  #
  # 代價是每次部署所有容器都會重起（幾秒）。但走到這一行就表示真的有新 commit,
  # 本來就該重起；用幾秒的確定性換掉一整類查不出來的問題，划算。
