<!--
Sync Impact Report
- Version change: 4.0.1 → 4.1.0
- Bump rationale: MINOR。放寬既有限制並新增條文，但不移除、也不重新定義任何原則。
  原則 VI「嵌入的第三方元件」原本五項條件之首是「自行 host，MUST NOT 以
  `<script src>` 指向外部主機」；本次為**跨來源 iframe** 這一種嵌入形式開了條件式
  的例外，並補上一組專屬於該形式的約束。原條件對「在我們的 document 裡執行」的
  嵌入**維持不變且一字未改**，故非 MAJOR。
- Modified principles:
  - II. 前後端分離的分層架構 →「例外：嵌入式第三方元件」補上一段，說明本例外
    涵蓋兩種形式。形式 B 底下讓瀏覽器連出去的是我們自己寫的 `<iframe src>`，
    字面上更接近該原則禁止的事，但**沒有任何資料流回前端**（跨來源讀不到 frame
    的內容），因此例外成立。同時劃出界線：一旦要從 frame **拿回**東西
    （`postMessage`、檢測結果、對方 API），即不再是嵌入而是前端自接第三方。
  - VI. 誠實標示模擬範圍 →「嵌入的第三方元件」一條改寫為**兩種形式分別規範**：
    - **形式 A（同一 document 內執行）**：條件與 v4.0.0 完全相同，未放寬。
    - **形式 B（跨來源 sandboxed iframe）**：新增。自行 host 不再是條件——那條
      規則保護的是「我們的頁面跑什麼程式」，而形式 B 底下外部程式根本不在我們的
      頁面上跑。相依圖查核同理：那份程式碼已不在版控裡，掃不到對方今天部署了什麼。
      改為五項新條件：具名單一來源、`sandbox` 必要與禁用旗標、MUST NOT 同時以
      任何方式把該元件載回本頁、逐項列出被暴露的訪客資訊、畫面標示。
- 本次修訂使既有實作不合規之處已於同次修正，無遺留：
  - `frontend/src/components/WithinReachFab.tsx` → 由注入 `<script>` 改為 iframe，
    `sandbox` 不含 `allow-top-navigation`。
  - `frontend/public/wr-widget.js`（自行 host 的建置產物，224KB）→ 已移除。
    留著會是第二個事實來源：沒有人在載、也沒有人在更新的一份程式碼。
  - `frontend/src/__tests__/embeddedWidget.test.ts` → 改守新的邊界（來源白名單、
    sandbox 旗標、無 script 注入路徑），不再掃已不存在的 bundle。
- ⚠️ 本次**確實放棄**了一項既有保障，記在這裡而不是只寫在程式碼註解裡：
  嵌入的內容會隨對方部署即時改變，我們無從得知、也無法回退到特定 commit。
  v4.0.0 的條文正是為了擋這件事。接受它的理由是影響範圍換小了——從
  「我們的頁面跑什麼程式（讀得到 JWT）」縮成「那塊畫面顯示什麼」。
  **這是換一種代價，不是消除代價**，MUST NOT 被複述為「改成 iframe 之後更安全了」。
- 依據：使用者於 2026-08-05 指示浮球改連 `https://within-reach-phi.vercel.app/`，
  並指示「改憲章讓這功能可以用」。
...
- Bump rationale: MAJOR。重新定義原則 VI 的一條，且該條原本明文寫著
  ...
  同時對原則 II 的「前端 MUST NOT 直接連線第三方」開了一個具名例外。
  三者任一都足以構成 MAJOR。
- Modified principles:
  - II. 前後端分離的分層架構 → 新增「例外：嵌入式第三方元件」一條。
    例外只涵蓋被嵌入元件自身的行為；**我們自己寫的程式碼**要取用外部資料，
    仍然 MUST 經後端。
  - VI. 誠實標示模擬範圍 → 「照片（前台使用者）」全條改寫：從「僅於瀏覽器內
    以 Canvas 處理」改為「前台 MUST NOT 向使用者索取任何私人照片」。
    保護的方式從「守住那條路徑」變成「不存在那條路徑」。
  - VI → 新增「嵌入的第三方元件」一條，五項條件全部 MUST：自行 host、
    來源可追溯、每次更新查核相依圖、觸網即改 sandboxed iframe 或移除、畫面標示。
  - VI →「模擬的外部整合」開頭加上「除上一條明文允許的嵌入式元件外」。
    渠道比價禁爬蟲、AI 評論審核禁呼叫 LLM 兩項**維持不變**——那兩條的理由是
    法律與倫理，與本次的技術決定無關。
- 本次修訂使既有實作不合規之處已於同次修正，無遺留：
  - `frontend/src/pages/RiskCheck.tsx`（自建的照片安全檢測）→ 已刪除。
  - `frontend/src/lib/__tests__/riskCheckIsolation.test.ts`（T144，靜態相依圖）
    與 `frontend/src/pages/__tests__/riskCheckNetwork.test.tsx`（T144a，執行期
    流量）→ 已刪除。兩支守的是已不存在的頁面。
  - `frontend/src/lib/riskScore.ts` 與 `riskScore.test.ts` → **保留**。
    後台房源品質檢測（`pages/admin/RoomRisk.tsx`）仍在使用同一條公式，
    那條路徑不在本次變更範圍內。
  - 新增 `frontend/src/pages/WithinReach.tsx` 與 `frontend/public/wr-widget.js`。
    ...
    相依圖查核結果：無 fetch／XHR／WebSocket／sendBeacon，
    無 localStorage／sessionStorage／cookie，畫面掛在 Shadow DOM。
    ⚠️ 已知例外：示範照片以 `<img>` 指向 `https://images.unsplash.com`，
    會將訪客 IP 與 referrer 暴露給該站。已於實機驗證中確認，記於
    `WithinReach.tsx` 檔頭；待上游改為打包本機資產後消除。
- 尚待同步（本次未處理，MUST 於後續修訂）：
  - `specs/001-booking-site/spec.md` 的 US9、FR-062、FR-065 ~ FR-067、FR-086、
    SC-015、SC-030 仍在描述已移除的自建檢測。
  - `specs/001-booking-site/tasks.md` 的 T144、T144a、T146。
- 依據：使用者於 2026-08-05 指示「檢測系統不自己架構，改嵌外部的系統」，
  並指定嵌入 Within Reach 的 `wr-widget.js` 打包檔。
...
- **例外：嵌入式第三方元件。** 經憲章明文允許嵌入的第三方前端元件（見原則 VI
  「嵌入的第三方元件」），其自身的行為不受上一條約束——它是一段跑在瀏覽器裡的
  外部程式，不是我們的資料存取路徑。但該例外 MUST NOT 被引申為「前端可以自己
  去接第三方 API」：**我們自己寫的程式碼**要取用外部資料，仍然 MUST 經後端。
  （v4.1.0 補充：本例外**涵蓋原則 VI 兩種嵌入形式**。形式 B 底下，讓瀏覽器連到
  第三方的是我們自己寫的那一行 `<iframe src>`，字面上更接近上一條禁止的事——
  但**它取不回任何資料**：跨來源之下我們讀不到那個 frame 的任何內容，它只是把
  一塊畫面讓出去。上一條防的是「資料繞過後端進到前端」，而這裡沒有資料流進來，
  所以例外成立。⚠️ 這也劃出了界線：哪天要從那個 frame **拿回**任何東西
  ——`postMessage`、回傳檢測結果、對方的 API——那就不再是嵌入，是前端自己接
  第三方，MUST 改走後端或 MUST 先修訂本條。）
...
- **個資**：MUST NOT 要求或儲存真實身分證字號、真實金融資訊。示範資料中的姓名與
  聯絡方式 MUST 為虛構。
- **照片（前台使用者）**：本平台的前台 **MUST NOT 向使用者索取任何私人照片**。
  自建的照片安全檢測已於 v4.0.0 移除，改嵌外部的無障礙檢測元件（見下條），
  該元件不接受上傳。日後若前台要恢復任何形式的使用者照片上傳，MUST 先修訂本條，
  MUST NOT 以「反正只在瀏覽器內處理」為由直接實作——那正是舊條文的內容，而它
  之所以能成立，是因為當時整條路徑都由本專案自己寫。
...
- **嵌入的第三方元件**：本專案 MAY 嵌入外部團隊提供的前端元件，取代自行實作。
  目前僅有一項：無障礙檢測（Within Reach，`components/WithinReachFab.tsx`）。
  嵌入 MUST 採下列兩種形式之一，並滿足該形式的**全部**條件，
  任一項不成立即 MUST NOT 嵌入。

  **⚠️ 兩種形式的差別只有一個，但那一個決定了其餘所有條件**：外部程式是跑在
  我們的 document 裡，還是跑在對方的 origin 上。前者的邊界是「我們讀完了它的
  程式碼」，後者的邊界是瀏覽器的同源政策。**MUST NOT 混用兩套條件**——挑對自己
  方便的那幾項湊成一份，會得到一個兩邊保障都沒有的安排。

  **形式 A｜在我們的 document 內執行**（`<script>`、Web Component、Shadow DOM）

  - **自行 host**：建置產物 MUST 進本專案版控並由本站台提供。
    MUST NOT 以 `<script src>` 指向外部主機——那等於把「這頁跑什麼程式」的
    決定權長期讓出，對方任何一次改版都會直接生效在正式站上而我們無從得知。
  - **來源可追溯**：MUST 記錄上游 repo、建置指令與來源 commit。
    更新版本 MUST 是一次可審閱、可回退的 commit。
  - **相依圖查核**：每次引入或更新版本，MUST 逐檔確認該元件的相依圖中
    是否存在 `fetch`／`XMLHttpRequest`／`WebSocket`／`sendBeacon` 與
    `localStorage`／`sessionStorage`／`document.cookie`，並將結果寫進程式碼註解。
    **本專案的 JWT 存於 `localStorage`，同一個 document 裡的第三方程式讀得到。**
    查核 MUST NOT 因為「上一版是乾淨的」而省略。
  - **升級路徑**：該元件一旦開始送出資料或觸及瀏覽器儲存，
    MUST 改以形式 B 隔離，或 MUST 移除。
    載入外部靜態資產（圖片、字型）不觸發本條，但 MUST 在程式碼註解中列出，
    MUST NOT 被含糊描述為「沒有外部請求」——那會把「沒有送出資料」與
    「沒有暴露訪客」混為一談，而後者仍然發生了。
  - **標示**：畫面上 MUST 標明該區塊由外部服務提供，
    MUST NOT 讓使用者以為那是 Sunny 自己的判定。

  **形式 B｜跨來源 sandboxed iframe**（v4.1.0 新增）

  - **來源具名且跨來源**：嵌入的 origin MUST 以單一具名常數寫死，並 MUST 與
    本站台不同源。⚠️ **同源內容 MUST NOT 用本形式**——同源之下 iframe 不構成
    任何邊界，卻會讓人以為構成了。自行 host 在本形式底下 MUST NOT 被要求：
    那條規則保護的是「我們的頁面跑什麼程式」，而這裡外部程式不在我們的頁面上跑。
  - **sandbox 為必要且有禁用清單**：`sandbox` 屬性 MUST 存在（沒有它不是
    「限制較少」，是完全沒有限制）。MUST NOT 含 `allow-top-navigation`
    及其變體——有了它，frame 內的程式可以把**我們的**整頁導向任何地方，
    而使用者在被導走之前，網址列上看到的一直是我們的網域。
    `allow-modals`、`allow-downloads`、`allow-pointer-lock` 同樣 MUST NOT 含。
    `allow-same-origin` MAY 給，因為跨來源之下它給的是**對方自己的** origin；
    ⚠️ 這一項若用在同源內容上等同解除整個沙箱，這正是上一條存在的理由。
  - **MUST NOT 有回本頁的載入路徑**：同一功能 MUST NOT 同時以形式 A 載入。
    ⚠️ 這是本形式最容易失守的一條——只要有任何一條路徑把外部程式載回本頁
    （舊寫法沒刪乾淨、或日後為了「這樣比較快」加回來），上面所有 `sandbox`
    旗標會一項都幫不上忙，而畫面看起來完全正常。
  - **暴露範圍逐項列出**：訪客的 IP 與 referrer 會給到被嵌入的主機及其載入的
    第三方。`referrerPolicy` 擋得住 referrer，擋不住 IP。這些 MUST 逐項列於
    程式碼註解，MUST NOT 因為「沒有資料被送出」就描述成不存在。
  - **標示內容為即時載入**：畫面上 MUST 標明該區塊由外部服務提供，且 MUST NOT
    讓使用者以為那是 Sunny 自己的判定。⚠️ 本形式下標示 MUST 另外反映
    **內容為即時載入、我們未留存副本**——因為這正是本形式相對形式 A 放棄的東西。
  - **已知放棄的保障**：本形式下，嵌入的內容會隨對方部署即時改變，我們無從得知，
    也無法回退到特定 commit。採用本形式即為接受這一點；
    MUST NOT 被複述為「改用 iframe 之後更安全了」——換掉的是代價的種類。
- **模擬的外部整合**：除上一條明文允許的嵌入式元件外，本專案不連接任何第三方
  商業平台。以下模組 MUST 以種子資料
...
**關於 v4.0.0 放寬第三方嵌入**：舊條文寫著「此條不可放寬」，而本次確實放寬了它。
記下理由，否則下一次有人想放寬別的條文時，會拿這次當先例。

舊條文保護的是**使用者的私人照片**，做法是自己實作、讓那條路徑上沒有任何出口。
新的安排沒有放寬那個保護，而是讓它變得不必要——前台不再向使用者要照片，
所以沒有東西需要被保護。這是把風險移除，不是接受風險。

真正新增的風險是另一個：一段外部程式碼跑在我們的頁面上，而我們的 JWT 就在
同一個 `localStorage` 裡。這個風險舊憲章沒有處理過，因此「嵌入的第三方元件」
一條把它寫成明確的條件——自行 host、來源可追溯、每次更新都重新查核相依圖。
放寬一條的同時補上另一條，是因為換掉的是風險的種類，不是風險的數量。

**關於 v4.1.0 新增形式 B**：上一段那三個條件，在 v4.1.0 被一個更省事的安排換掉了
——直接嵌對方的線上部署。這裡同樣要記下代價，理由和上一段一樣：**下一次有人想
「反正 iframe 比較安全」而略過查核時，會拿這次當先例。**

換到的是真的：外部程式不再跑在我們的 document 裡，於是它讀不到 `localStorage`
裡的 JWT——**而那正是形式 A 那三個條件唯一在保護的東西**。保護消失了，是因為
被保護的目標不再暴露，不是因為我們決定不保護它。

放棄的也是真的，而且正好是 v4.0.0 最在意的那一項：內容隨對方部署即時改變，
我們無從得知、無法回退。差別在**失守時的爆炸半徑**——形式 A 底下對方一次改版
可以拿走我們使用者的 token，形式 B 底下只能改變那塊畫面顯示什麼。

因此形式 B 的五項條件全部圍繞著「維持那道 origin 邊界」，而不是「檢查對方寫了
什麼」。其中「MUST NOT 有回本頁的載入路徑」是整組的樞紐：那道邊界一旦破了，
形式 B 就退化成一個沒有做過任何查核的形式 A。
...
**Version**: 4.1.0 | **Ratified**: 2026-07-30 | **Last Amended**: 2026-08-05
