...
├── index.html
├── public/
│   └── wr-widget.js          # 嵌入式第三方元件的建置產物（自行 host，FR-129）
└── src/
...
**新的風險在別處**：前台那個位置改嵌外部元件，而本專案的 JWT 就在同一個 document 的
`localStorage` 裡。約束見 FR-129 ~ FR-131，任務見 T184、T185。
