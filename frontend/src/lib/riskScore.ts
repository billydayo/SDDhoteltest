 ...
 *
 * ⚠️ **本模組不做任何網路存取，也 MUST NOT 引入會做網路存取的模組。**
 *
 * 前台的「安全檢測」（US9）讓使用者上傳自己的私人照片，而那些照片 MUST 全程
 * 留在瀏覽器內、MUST NOT 送往任何外部服務或長期儲存（FR-066、FR-086、SC-030）。
 * 保證這件事的方式不是「小心不要呼叫 API」，而是**讓 `pages/RiskCheck.tsx`
 * 的相依圖裡只有這一個檔案**——它沒有任何出口。T144 與 T144a 兩支測試分別從
 * 靜態相依與執行期流量兩面守著。
 *
 * 後台的房源品質檢測（US9 的另一半）會把結果送上去，但**那條路徑的上傳程式
 * 不在這裡**（`pages/admin/RoomRisk.tsx` 自己呼叫 `api`）。兩條路徑共用
 * 「計算」，不共用「上傳」。
 *
 * ## 公式
 *
