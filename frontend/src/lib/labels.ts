 ...
 * 付款方式（FR-026）。
 *
 * ⚠️ **三者皆為模擬支付**，不涉及任何真實金流。介面上顯示這些名稱的地方
 * MUST 同時標明「虛擬支付，不會產生任何實際交易」（FR-027）——名稱本身
 * 看起來與真實的付款選項一模一樣，那正是需要標示的原因。
 *
 * `LINE Pay` 的鍵與值相同：它是專有名詞，沒有中文寫法。留在表裡而不是特例
 * 處理，是為了讓「三種付款方式」在這一份資料上就數得出來。
 */
...
export const paymentMethodLabel = (value: string) => lookup(PAYMENT_METHOD, value)
